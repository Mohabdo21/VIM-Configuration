return {
	...
	config = function()
		...
		local function dim_color(hex, bg_hex, ratio)
			ratio = ratio or 0.3
			local r1, g1, b1 = tonumber(hex:sub(2, 3), 16), tonumber(hex:sub(4, 5), 16), tonumber(hex:sub(6, 7), 16)
			local r2, g2, b2 =
				tonumber(bg_hex:sub(2, 3), 16), tonumber(bg_hex:sub(4, 5), 16), tonumber(bg_hex:sub(6, 7), 16)
		...
		local my_lualine_theme = {
			normal = {
				...
				b = { bg = dim_color(colors.blue, colors.bg), fg = colors.fg },
				c = { bg = colors.bg, fg = colors.fg },
			},
			insert = {
				a = { bg = colors.green, fg = colors.bg, gui = "bold" },
				b = { bg = dim_color(colors.green, colors.bg), fg = colors.fg },
				c = { bg = colors.bg, fg = colors.fg },
			},
			visual = {
				a = { bg = colors.violet, fg = colors.bg, gui = "bold" },
				b = { bg = dim_color(colors.violet, colors.bg), fg = colors.fg },
				c = { bg = colors.bg, fg = colors.fg },
			},
			command = {
				a = { bg = colors.yellow, fg = colors.bg, gui = "bold" },
				b = { bg = dim_color(colors.yellow, colors.bg), fg = colors.fg },
				c = { bg = colors.bg, fg = colors.fg },
			},
			replace = {
				a = { bg = colors.red, fg = colors.bg, gui = "bold" },
				b = { bg = dim_color(colors.red, colors.bg), fg = colors.fg },
				c = { bg = colors.bg, fg = colors.fg },
			},
			inactive = {
